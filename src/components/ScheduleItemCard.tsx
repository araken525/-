...
export default function ScheduleItemCard({
  ...
  return (
    // ★修正: 外側の箱 (バッジの表示領域を確保するため、overflowは指定しない)
    <div className={`relative h-full ${now ? "z-10" : ""}`}>
      
      {/* NOWバッジ (カードの外に飛び出すので、ここに配置) */}
      {now && (
